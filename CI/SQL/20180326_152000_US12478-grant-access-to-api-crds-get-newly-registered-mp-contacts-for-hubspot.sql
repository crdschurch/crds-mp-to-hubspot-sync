...
begin
	...
	if not exists(select 1 from dbo.dp_API_Procedures where [Procedure_Name] = 'api_crds_get_newly_registered_mp_contacts_for_hubspot')
	begin
		...
		values(
            'api_crds_get_newly_registered_mp_contacts_for_hubspot',
			'Fetches newly registered Ministry Platform contact data to be synced to the Crossroads HubSpot CRM instance.'
		);
