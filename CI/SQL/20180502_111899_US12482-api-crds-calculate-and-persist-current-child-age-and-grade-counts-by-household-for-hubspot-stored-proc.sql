...
as
    ...
    begin
        ...
        begin -- nothing to do, so commit now
            ...
            select ProcessedUtc, SyncCompletedUtc, InsertCount, UpdateCount from dbo.cr_ChildAgeAndGradeDeltaLog where ProcessedUtc = @ProcessedUtc;
        end
        else
        begin
            insert into dbo.cr_ChildAgeAndGradeDeltaLog (ProcessedUtc, SyncCompletedUtc, CreatedUtc, LastModifiedUtc, InsertCount, UpdateCount)
    ...
    select ProcessedUtc, SyncCompletedUtc, InsertCount, UpdateCount from dbo.cr_ChildAgeAndGradeDeltaLog where ProcessedUtc = (select max(ProcessedUtc) from dbo.cr_ChildAgeAndGradeDeltaLog where SyncCompletedUtc is null);
